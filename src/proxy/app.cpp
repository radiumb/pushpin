...
public:
	...
	void start()
	{
		...
		long group_byte_count = 4; // for group byte count
		...
		// Cache
		long cache_byte_count = 4; // for cache byte count
		...
		// Cache Subscribe
		long cache_subscribe_byte_count = 4; // for cache subscribe byte count
		...
		for (long i = 0; i < cache_method_count; i++)
		{
		...
		if (pid == -1){
			...
			// create wscat
			char * argv_list1[] = {cacheClientBin.toLocal8Bit().data(), cacheClientOption.toLocal8Bit().data(), cacheClientUrl.toLocal8Bit().data(), "-H Socket-Owner:Cache_Client", NULL};
